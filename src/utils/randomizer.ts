...
export function generateBuild(filters: FilterState): Build {
  ...
  const selectedWeapons = getRandomElements(availableWeapons, Math.min(4, availableWeapons.length));

  const selectedTomes = getRandomElements(availableTomes, Math.min(4, availableTomes.length));
